...
export default function AdminDashboard() {
  ...
  const handleSeedDatabase = async () => {
    ...
    setErrorMsg(null);
    try {
      const res = await fetch("http://localhost:8000/api/setup.php");
  ...
  const handleDeleteMessage = async (id: number) => {
    ...
    if (dbConnected) {
      try {
        const res = await fetch(`http://localhost:8000/api/messages.php?id=${id}`, {
          method: "DELETE"
        });
        if (!res.ok) throw new Error("Delete request failed");
        loadData();
